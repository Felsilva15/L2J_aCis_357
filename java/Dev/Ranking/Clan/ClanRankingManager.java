	...
	static int getClanRanking1stPlayerReward()
	{
		     ...
		     ResultSet rset = statement.executeQuery())
		{
			if (rset.next())
			{
				...
				{
					leaderId.setInt(1, clanId);
					
					try (ResultSet result2 = leaderId.executeQuery())
					{
						if (result2.next())
						{
	...
	static int getClanRanking2ndPlayerReward()
	{
	         ...
	         ResultSet rset = statement.executeQuery())
	    {
	        while (rset.next())
	        {
	            ...
	            try (PreparedStatement leaderId = con.prepareStatement("SELECT leader_id FROM clan_data WHERE clan_id=" + id);
