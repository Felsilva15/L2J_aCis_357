...
public class FOSEvent
{
	...
	public static boolean startFight()
	{
		...
		// Setinstance Doors
		instanceDoors(FOSConfig.FOS_DOORS_ATTACKABLE);
	...
	private static void SpawnArtifact()
	{
...
//			}
	...
	public static boolean isDoorAttackable(int id, Creature attacker)
	{
		for (int doorId : FOSConfig.FOS_DOORS_ATTACKABLE)
		{
			...
			if (attacker instanceof Player && ((Player)attacker).isInFOSEvent())
				return true;
			else if (attacker instanceof Summon && ((Summon)attacker).getOwner().isInFOSEvent())
				return true;
			else if (attacker instanceof Pet && ((Pet)attacker).getOwner().isInFOSEvent())
				return true;
		}
		return false;
	}
	
	private static void instanceDoors(List<Integer> doors)
	{
	//	for (int doorId : doors)
		...
		//		doorInstance.setInstanceId(FOSConfig.FOS_INSTANCE_ID, true);
		}
	}
	...
	//		Door doorInstance = DoorTable.getInstance().getDoor(doorId);
		...
		//	if (doorInstance != null)
		///		doorInstance.setInstanceId(0, true);
		//}
	}
	...
	public static boolean checkIfOkToCastSealOfRule(Player player)
	{
		...
		if (player.getTarget() instanceof Folk && ((Folk)player.getTarget())._isFOS_Artifact && player.isInFOSEvent())
			return true;
