...
import net.sf.l2j.events.manager.EventTask;
...
public class CTF implements EventTask
{
	...
	private static boolean addPlayerOk(final String teamName, final Player eventPlayer)
	{
		...
		else if (FOSEvent.isPlayerParticipant(eventPlayer.getObjectId()))
		{
			eventPlayer.sendMessage("You already participated in another event!");
			return false;
		}
		else if (eventPlayer.isInArenaEvent())
		{
			eventPlayer.sendMessage("You already participated in another event!");
			return false;
		}
		else if (DMEvent.isPlayerParticipant(eventPlayer.getObjectId()))
		{
			eventPlayer.sendMessage("You already participated in another event!");
			return false;
		}
		else if (KTBEvent.isPlayerParticipant(eventPlayer.getObjectId()))
		{
			eventPlayer.sendMessage("You already participated in another event!");
			return false;
		}
