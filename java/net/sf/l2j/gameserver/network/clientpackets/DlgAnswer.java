...
//		}
		...
		{
			if (activeChar.isOff() || TvTEvent.isPlayerParticipant(activeChar.getObjectId()) && TvTEvent.isStarted() || activeChar.isOffShop() || FOSEvent.isPlayerParticipant(activeChar.getObjectId()) && FOSEvent.isStarted() || DMEvent.isPlayerParticipant(activeChar.getObjectId()) && DMEvent.isStarted() || KTBEvent.isPlayerParticipant(activeChar.getObjectId()) && KTBEvent.isStarted() ||  CTF.is_started() && activeChar._inEventCTF || activeChar._inEventCTF  || activeChar.isAio() || activeChar.isInsideZone(ZoneId.PVP_CUSTOM) || activeChar.isDead()  /*KTBEvent.isPlayerParticipant(activeChar.getObjectId()) && KTBEvent.isStarted()*/ || activeChar.isAlikeDead() || activeChar.isInArenaEvent() || activeChar.isArenaProtection() || activeChar.isOlympiadProtection() || activeChar.isInStoreMode() || activeChar.isRooted() || activeChar.getKarma() > 0 || activeChar.isInOlympiadMode() || activeChar.isFestivalParticipant() || activeChar.isArenaAttack() || activeChar.isInsideZone(ZoneId.BOSS) || activeChar.isInsideZone(ZoneId.ARENA_EVENT) || activeChar.isInsideZone(ZoneId.PVP_CUSTOM) || activeChar.isInsideZone(ZoneId.SIEGE) || activeChar.isInsideZone(ZoneId.TOURNAMENT))
			{
				return;
			}
