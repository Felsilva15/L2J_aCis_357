...
public class RaidZoneManager
{
    ...
    private static boolean checkPlayersKickTask(Player activeChar, Integer numberBox)
    {
        ...
        if (activeChar != null)
        {
        	...
        	{
        //		if (!player.isInsideZone(ZoneId.RAID_ZONE) || player.getHWID() == null)
    ...
    public boolean checkPlayersArea(Player activeChar, Integer numberBox, Boolean forcedTeleport)
    {
        if (checkPlayersKickTask(activeChar, numberBox))
        {
            if (forcedTeleport)
            {
            	activeChar.sendPacket(new CreatureSay(0, Say2.TELL, "SYS","Allowed only " + numberBox + " Client in Raid Zone!"));
				for (Player allgms : AdminData.getInstance().getAllGms(true))
				{
					if (!activeChar.isGM())
						allgms.sendPacket(new CreatureSay(0, Say2.TELL, "[Double HWID]", activeChar.getName() +" in Raid Zone!"));
				}
    ...
    private static boolean checkPlayersKickTask_ip(Player activeChar, Integer numberBox, Collection<Player> world)
    {
        ...
        //    if (!player.isInsideZone(ZoneId.RAID_ZONE) || player.getHWID() == null || player.getClient().isDetached())
        	  if (!player.isInsideZone(ZoneId.RAID) || !player.isInsideZone(ZoneId.RAID_NO_FLAG) || player.getHWID() == null || player.getClient().isDetached())
                continue;
