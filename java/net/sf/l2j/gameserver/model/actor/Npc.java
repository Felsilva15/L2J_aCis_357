...
public class Npc extends Creature
{
	...
	private static final int[] galaxia =
	{
		25450
	};
		...
		else
		{
			...
			else
			{
				...
				//	else if (_isEventMobCTF)
					...
					else if (!KTBEvent.isPlayerParticipant(player.getObjectId()) && KTBEvent.isStarted() && getNpcId() == KTBConfig.KTB_EVENT_PARTICIPATION_NPC_ID)
					{
...
//	}
	...
	public int getExpReward()
	{
	    double expReward = getTemplate().getRewardExp();
	    
	    // Se Config.ENABLE_XP_SP_DOUBLE for verdadeiro e for final de semana, dobra a taxa de XP
	    if (Config.ENABLE_XP_SP_DOUBLE && isWeekend())
	    {
	        expReward *= 2;  // Dobra o XP nos finais de semana, caso a configuração permita
	    }
	    
	    return (int) (expReward * Config.RATE_XP);  // Aplica a taxa de XP padrão
	}
	...
	public int getSpReward()
	{
	    double spReward = getTemplate().getRewardSp();
	    
	    // Se Config.ENABLE_XP_SP_DOUBLE for verdadeiro e for final de semana, dobra a taxa de SP
	    if (Config.ENABLE_XP_SP_DOUBLE && isWeekend())
	    {
	        spReward *= 2;  // Dobra o SP nos finais de semana, caso a configuração permita
	    }
	    
	    return (int) (spReward * Config.RATE_SP);  // Aplica a taxa de SP padrão
	}
