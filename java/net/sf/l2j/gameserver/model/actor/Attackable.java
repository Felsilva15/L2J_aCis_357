...
/*	private IntIntHolder calculateRewardItem(Player lastAttacker, DropData drop, int levelModifier, boolean isSweep)
	...
	/**
	 * Calculates the reward item drop for a player.
	 * @param lastAttacker The player who last attacked the NPC.
	 * @param drop The drop data.
	 * @param levelModifier The level modifier of the NPC.
	 * @param isSweep If it is a spoil event.
	 * @return the item drop or null if no item is dropped.
	 */
	private IntIntHolder calculateRewardItem(Player lastAttacker, DropData drop, int levelModifier, boolean isSweep)
	{
	    // Get default drop chance
	    ...
	    // Check if deep blue drop rules are enabled
	    if (Config.DEEPBLUE_DROP_RULES)
	    {
	        int deepBlueDrop = 1;
	        if (levelModifier > 0)
	        ...
	        // Apply drop chance based on deep blue rules
	        dropChance = ((drop.getChance() - ((drop.getChance() * levelModifier) / 100)) / deepBlueDrop);
	    }
	    
	    // Applies Drop rates (considering weekend boosts)
	    if (Config.ENABLE_XP_SP_DOUBLE && isWeekend())
	    {
	        // Apply weekend rates if it's Saturday or Sunday and XP/SP Double is enabled
	        if (Config.DROP_LIST_VIP.contains(drop.getItemId()) && lastAttacker.isVip() && Config.ENABLE_DROP_VIP)
	            dropChance *= (Config.RATE_DROP_VIP * 2);  // Double the VIP rate for weekends
	        else if (drop.getItemId() == 57 && lastAttacker.isVip() && Config.ENABLE_DROP_VIP)
	            dropChance *= (Config.RATE_DROP_ADENA * Config.RATE_DROP_VIP * 2);  // Double the adena VIP rate for weekends
	        else if (drop.getItemId() == 57)
	            dropChance *= (Config.RATE_DROP_ADENA * 2);  // Double the adena drop rate for weekends
	        else if (isSweep)
	            dropChance *= (Config.RATE_DROP_SPOIL * 2);  // Double the spoil drop rate for weekends
	        else
	            dropChance *= (Config.RATE_DROP_ITEMS * 2);  // Double the item drop rate for weekends
	    } 
	    else
	    {
	        // Normal drop rates for weekdays or when XP_SP_DOUBLE is disabled
	        if (Config.DROP_LIST_VIP.contains(drop.getItemId()) && lastAttacker.isVip() && Config.ENABLE_DROP_VIP)
	            dropChance *= Config.RATE_DROP_VIP;
	        else if (drop.getItemId() == 57 && lastAttacker.isVip() && Config.ENABLE_DROP_VIP)
	            dropChance *= (Config.RATE_DROP_ADENA * Config.RATE_DROP_VIP);
	    ...
	    // Ensure raid drops are not increased for weekends
	    if (isRaid() && !isRaidMinion()) 
	    {
	        // Ensure that raid drops always follow the original drop rate (1x), regardless of the weekend
		...
		if (Config.CUSTOM_DROP && Config.LIST_NPC_CUSTOM_MONSTER.contains(Integer.valueOf(npcTemplate.getNpcId()))) {
			if (player.getParty() != null) {
				...
				for (int[] reward : Config.CUSTOM_MONSTER_DROP) {
					if (ItemTable.getInstance().createDummyItem(reward[0]).isStackable()) {
						if (player.isVip()) {
							player.addItem("l2server", reward[0], Rnd.get(reward[1], reward[1] * 2) * Config.RATE_DROP_VIP, player, true);
							continue;
						} 
						player.addItem("l2server", reward[0], Rnd.get(reward[1], reward[1] * 2), player, true);
					} 
				} 
			} 
			return;
			...
			for (int[] reward : Config.CHAMPION_INVADE_DROP) 
			{
				if (ItemTable.getInstance().createDummyItem(reward[0]).isStackable()) 
				{
					if (player.isVip() && Config.VIP_REWARD_BONUS_INVADE)
						player.addItem("aCis", reward[0], Rnd.get(reward[1], reward[1] * 1) * Config.RATE_DROP_VIP, player, true);
					else
