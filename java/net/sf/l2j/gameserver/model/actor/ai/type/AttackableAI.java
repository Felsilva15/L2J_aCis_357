...
//		if (attackTarget == null || _attackTimeout < System.currentTimeMillis() || MathUtil.calculateDistance(npc, attackTarget, true) > 2000 && !(npc.getNpcId() == Config.NPC_ANT_PK))
//		{
...
//		}
	protected void thinkAttack()
	{
		...
		// If target doesn't exist, is too far or if timeout is expired.
	//	if (attackTarget == null || _attackTimeout < System.currentTimeMillis() || MathUtil.calculateDistance(npc, attackTarget, true) > 2000)
		if (attackTarget == null || _attackTimeout < System.currentTimeMillis() || MathUtil.calculateDistance(npc, attackTarget, true) > 2000 && !(npc.getNpcId() == 9999))
		{
			// Stop hating this target after the attack timeout or if target is dead
			npc.stopHating(attackTarget);
			setIntention(CtrlIntention.ACTIVE);
	...
	/* RAID BOSS BUG	else if ((npc instanceof RaidBoss) && (attackTarget instanceof Player) && !npc.isDead() && Config.LIST_RAID_BOSS_ZONE_IDS.contains(npc.getNpcId()) && (!npc.isInsideZone(ZoneId.RAID) || !attackTarget.isInsideZone(ZoneId.RAID)) && (!npc.isInsideZone(ZoneId.RAID_NO_FLAG) || !attackTarget.isInsideZone(ZoneId.RAID_NO_FLAG)) && !npc.getSpawn().is_customBossInstance())
		{
			setIntention(CtrlIntention.ACTIVE);
			npc.setWalking();
			npc.abortAttack();
...
//EPIC BOSS BUG		else if ((npc instanceof GrandBoss) && (attackTarget instanceof Player) && !npc.isDead() && npc.getNpcId() == 29001 && (!npc.isInsideZone(ZoneId.RAID) || !attackTarget.isInsideZone(ZoneId.RAID)) && (!npc.isInsideZone(ZoneId.RAID_NO_FLAG) || !attackTarget.isInsideZone(ZoneId.RAID_NO_FLAG)))
		{
			if (!npc.isMinion() && !npc.isRaidMinion())
			{
				setIntention(CtrlIntention.ACTIVE);
