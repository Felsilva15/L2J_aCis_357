...
public class Announcement implements Runnable
{
	...
	{
		CreatureSay cs = new CreatureSay(0, Say2.ANNOUNCEMENT, "", "" + msg);
...
//				
//				if (!(player.isOlympiadProtection() || player.isCursedWeaponEquipped() || player.isInObserverMode() || player.getFirstLog() || player._inEventCTF || player._inEventTvT || player.isInsideZone(ZoneId.PVP_CUSTOM) || player.isInsideZone(ZoneId.RAID) || player.isInsideZone(ZoneId.RAID_NO_FLAG) || player.isInsideZone(ZoneId.PARTYZONE) || player.isAio() || bishop) && Config.SCREN_MSG_PVP)
//				{
	...
	public static void PvPAnnounce(String text)
	{
		...
		for (Player player : World.getInstance().getPlayers())
		{
			if (player != null && player.isOnline())
			{
				...
				final boolean bishop = (player.getClassId() == ClassId.BISHOP || player.getClassId() == ClassId.CARDINAL
					|| player.getClassId() == ClassId.SHILLIEN_ELDER || player.getClassId() == ClassId.SHILLIEN_SAINT
					|| player.getClassId() == ClassId.EVAS_SAINT || player.getClassId() == ClassId.ELVEN_ELDER);
				...
				if (!(player.isOlympiadProtection()
					...
					|| player.isAio()
					|| player.isOff() // NOVO
					|| player.isOffShop() // NOVO
					|| bishop)
					&& Config.SCREN_MSG_PVP)
				{
					SpawnLocation _position = new SpawnLocation(
						Config.pvp_locx + Rnd.get(-100, 100),
						Config.pvp_locy + Rnd.get(-100, 100),
						Config.pvp_locz,
						0);
