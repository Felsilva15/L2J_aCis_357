...
//	}
	private void load()
	{
	    ...
	    if (!loaded)
	    {
	        _log.info("Olympiad System: Failed to load data from database, default values are being used.");
	        _currentCycle = 1;
	        _period = 0;
	        _olympiadEnd = 0;
	        _validationEnd = 0;
	    ...
	    switch (_period)
	    {
	        case 0: // Competition period
	            if (_olympiadEnd == 0 || _olympiadEnd < now)
	            {
	                if (Config.ALT_OLY_USE_CUSTOM_PERIOD_SETTINGS)
	                    setNewOlympiadEndCustom();  // Custom period
	                else
	                    setNewOlympiadEnd();  // Default period

	                _log.info("Olympiad System: The next Olympiad period will start at " + new Date(_olympiadEnd));
	            }
	            else
	            {
	                scheduleWeeklyChange();
	                _log.info("Olympiad System: Olympiad period is in progress. Next review at " + new Date(_olympiadEnd));
	            }
	            break;

	        case 1: // Validation period
	            if (_validationEnd > now)
	            {
	                loadNoblesRank();
	                _scheduledValdationTask = ThreadPool.schedule(new ValidationEndTask(), getMillisToValidationEnd());
	                _log.info("Olympiad System: Validation period is ongoing. It will end at " + new Date(_validationEnd));
	            }
	            else
	            {
	                _currentCycle++;
	                ...
	                _log.info("Olympiad System: Validation period ended. New Olympiad period will start at " + new Date(_olympiadEnd));
	            }
	            break;

	        default:
	            _log.warning("Olympiad System: Invalid period loaded: " + _period);
	            return;
	    }
	         ...
	         ResultSet rset = statement.executeQuery())
	    {
	        while (rset.next())
	        {
	            ...
	            statDat.set(COMP_DRAWN, rset.getInt(COMP_DRAWN));
	    ...
	    {
	        _log.warning("Olympiad System: Error loading nobles data from database: " + e);
	    }

	    synchronized (this)
	    {
	        _log.info("Olympiad System: Loading Olympiad system...");
	        _log.info("Olympiad System: Current period is " + (_period == 0 ? "Competition" : "Validation"));

	        long milliToEnd = (_period == 0) ? getMillisToOlympiadEnd() : getMillisToValidationEnd();

	        int numDays = (int) (milliToEnd / (1000 * 60 * 60 * 24));
	        int numHours = (int) ((milliToEnd / (1000 * 60 * 60)) % 24);
	        int numMins = (int) ((milliToEnd / (1000 * 60)) % 60);

	        _log.info("Olympiad System: Time remaining until period ends: " + numDays + " days, " + numHours + " hours, and " + numMins + " minutes.");

	        if (_period == 0)
	        {
	            long milliToWeekChange = getMillisToWeekChange();

	            int daysWeek = (int) (milliToWeekChange / (1000 * 60 * 60 * 24));
	            int hoursWeek = (int) ((milliToWeekChange / (1000 * 60 * 60)) % 24);
	            int minsWeek = (int) ((milliToWeekChange / (1000 * 60)) % 60);

	            _log.info("Olympiad System: Next weekly change in " + daysWeek + " days, " + hoursWeek + " hours, and " + minsWeek + " minutes.");
	        }
	    }
	...
	public void announcePeriod(Player player)
	{
		...
		if (_validationEnd == 0 || _period == 0)
			player.sendPacket(new CreatureSay(0, Say2.PARTYROOM_ALL, "", "Olympiad ends in " + numDays + " day(s), " + numHours + " hour(s), " + numMins + " minute(s)."));
		else
