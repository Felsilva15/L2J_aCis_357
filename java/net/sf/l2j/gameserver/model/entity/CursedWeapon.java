...
//	}
	private void removeFromDb() {
	    try (Connection con = ConnectionPool.getConnection();
	        ...
	        if (rowsAffected == 0) {
	            // Log apenas se o item não foi encontrado e removido
	            _log.warning("CursedWeapon: No rows affected for itemId " + _itemId);
	        }
	    } catch (SQLException e) {
	        Throwable cause = e.getCause();
	        if (cause instanceof InterruptedException) {
	            Thread.currentThread().interrupt(); // Repassa o status de interrupção
	            _log.warning("CursedWeapon: Thread interrupted during DB removal: " + cause.getMessage());
	        } else {
	            _log.log(Level.SEVERE, "CursedWeapon: Failed to remove data for itemId " + _itemId, e);
	        }
	    }
	}
