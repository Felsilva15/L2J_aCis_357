...
public class Clan
{
	...
	public Clan(int clanId, int leaderId)
	{
		...
		loadRankingPoints();
		ClanRankingManager.getInstance().addClanRanking(_clanId, _champPoints + _bossPoints);
	...
	protected void loadRankingPoints()
	{
		...
		{
			LOGGER.warn("error while loading clan points from db " + e);
		} 
	}
	...
	private void storeRankingPoints() 
	{
	    try (Connection con = ConnectionPool.getConnection()) 
	    {
	        PreparedStatement statement = con.prepareStatement("UPDATE clan_points SET champ_points=?, boss_points=? WHERE clan_id=?");
	    ...
	    {
	        LOGGER.warn("Error While saving Ranking Points for Clan " + getName());
	    }
	    ClanRankingManager.getInstance().addClanRanking(_clanId, _champPoints + _bossPoints); // Ajuste aqui: não inclui mais siegePoints
	}
	...
	private void createPointsRecord(int clan_id)
	{
	    try (Connection con = ConnectionPool.getConnection())
	    {
	        ...
	        // Alterar a query para não incluir siege_points
	        PreparedStatement pointsStatment = con.prepareStatement("INSERT INTO clan_points(clan_id, champ_points, boss_points) VALUES(?,?,?)");
	        pointsStatment.setInt(1, _clanId);
	        pointsStatment.setInt(2, 0); // Inicializando champ_points
	        pointsStatment.setInt(3, 0); // Inicializando boss_points
	        pointsStatment.execute();
	        pointsStatment.close();
	    } 
	    catch (Exception e)
	    {
	        LOGGER.warn("Error while saving new clan points to DB: " + e);
	    } 
	}
	...
	{
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_CLAN))
		{
	...
	public void store()
	{
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(STORE_CLAN))
		{
			ps.setInt(1, _clanId);
	...
	private void storeNotice(String notice, boolean enabled)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_NOTICE))
		{
	...
	public void setIntroduction(String intro, boolean saveOnDb)
	{
		if (saveOnDb)
		{
			...
			try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_INTRODUCTION))
			{
	...
	public SubPledge createSubPledge(Player player, int type, int leaderId, String subPledgeName)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(INSERT_SUBPLEDGE))
		{
			ps.setInt(1, _clanId);
	...
	{
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_SUBPLEDGE))
		{
	...
	public void setPriviledgesForRank(int rank, int privs)
	{
		...
		// Update database.
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(ADD_OR_UPDATE_PRIVILEDGE))
		{
			ps.setInt(1, _clanId);
			ps.setInt(2, rank);
			ps.setInt(3, privs);
	...
	private void setReputationScore(int value)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_CRP))
		{
	...
	public void setAuctionBiddedAt(int id)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_AUCTION))
		{
			ps.setInt(1, id);
			ps.setInt(2, _clanId);
			ps.executeUpdate();
	...
	{
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_CLAN_LEVEL))
		{
			ps.setInt(1, level);
			ps.setInt(2, _clanId);
			ps.executeUpdate();
	...
	public void changeClanCrest(int crestId)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_CLAN_CREST))
		{
			ps.setInt(1, crestId);
			ps.setInt(2, _clanId);
			ps.executeUpdate();
	...
	public void changeAllyCrest(int crestId, boolean onlyThisClan)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(query))
		{
			ps.setInt(1, crestId);
			ps.setInt(2, allyId);
			ps.executeUpdate();
	...
	public void changeLargeCrest(int crestId)
	{
		...
		try (Connection con = ConnectionPool.getConnection(); PreparedStatement ps = con.prepareStatement(UPDATE_LARGE_CREST))
		{
			ps.setInt(1, crestId);
			ps.setInt(2, _clanId);
			ps.executeUpdate();
