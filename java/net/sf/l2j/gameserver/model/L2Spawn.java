...
public final class L2Spawn implements Runnable
{
	...
	public Npc doSpawn(boolean isSummonSpawn)
	{
		...
		//		((Creature) tmp).setShowSummonAnimation(isSummonSpawn);
			if (isSummonSpawn)
			{
				...
				else if (tmp instanceof AgathionNpc)
				{
					((AgathionNpc) tmp).setShowSummonAnimation(isSummonSpawn);
				}
			}
	...
	private void initializeAndSpawn()
	{
		...
		if (Config.CHAMPION_FREQUENCY > 0)
		{
			...
			if (_npc instanceof Monster && !getTemplate().cantBeChampion() && _npc.getLevel() >= Config.CHAMP_MIN_LVL && _npc.getLevel() <= Config.CHAMP_MAX_LVL && !_npc.isRaid() && !((Monster) _npc).isRaidMinion() && !((Monster) _npc).isMinion() && !_npc.isAgathion())
		...
		if(ChampionInvade.is_started())
		{
			if (Config.CHAMPION_INVADE_FREQUENCY > 0 && Config.CHAMPION_FARM_BY_TIME_OF_DAY)
			{
				...
				((Attackable) _npc).setChampionInvade(Rnd.get(100) < Config.CHAMPION_INVADE_FREQUENCY);
			}
		}
		else if(ChampionInvade.is_finish() && Config.LIST_NPC_CHAMPION_MONSTER.contains(Integer.valueOf(_template.getNpcId())) )
		{
			((Attackable) _npc).setChampionInvade(Rnd.get(100) < 0);
		}
