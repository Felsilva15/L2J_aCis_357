...
import net.sf.l2j.gameserver.RestartLogin;
...
public class L2LoginServer
{
	...
	{		
		...
		if(Config.RESTART_BY_TIME_OF_DAY_LOGIN)
		{
			RestartLogin.getInstance().StartCalculationOfNextRestartTime();
		}
		else
		{
			_log.info("# Auto Restart Login System is Disabled #");
		}
	}
	...
	{
		final File banFile = new File("config/Banned_ip.properties");
		if (banFile.exists() && banFile.isFile())
		{
				...
				while ((line = reader.readLine()) != null)
				{
					...
					if (line.length() > 0 && line.charAt(0) != '#')
					{
						...
						if (parts.length > 1)
							...
							{
								L2LoginServer._log.warning("Skipped: Incorrect ban duration (" + parts[1] + ") on Banned_ip.properties. Line: " + reader.getLineNumber());
								continue;
							}
						try
						{
						...
						{
							L2LoginServer._log.warning("Skipped: Invalid address (" + parts[0] + ") on Banned_ip.properties. Line: " + reader.getLineNumber());
						}
					}
				}
			}
			...
			{
				L2LoginServer._log.warning("Error while reading Banned_ip.properties. Details: " + e.getMessage());
				if (Config.DEVELOPER)
					e.printStackTrace();
			}
			L2LoginServer._log.info("Loaded " + LoginController.getInstance().getBannedIps().size() + " IP(s) from Banned_ip.properties.");
		}
		else
			L2LoginServer._log.warning("Banned_ip.properties is missing. Ban listing is skipped.");
	}
